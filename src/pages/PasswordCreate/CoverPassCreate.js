...
const CoverPasswCreate = () => {
  document.title = "Create New Password | Griphold";
  ...
  const validation = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        // Send a request to the server to initiate the forgot password process
        const response = await fetch(
          `${
            process.env.REACT_APP_API_URL
          }/api/auth/reset-pass?token=${encodeURIComponent(token)}`,
          {
            method: "POST",
            headers: {
  ...
  return (
    <React.Fragment>
      <div className=" py-5 d-flex justify-content-center align-items-center min-vh-100">
        <div className=""></div>
