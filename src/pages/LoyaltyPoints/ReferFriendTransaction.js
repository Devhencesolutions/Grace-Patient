...
const ReferTransactions = () => {
    ...
    const fetchUsers = async () => {
        ...
        await axios
            .post(
                `${process.env.REACT_APP_API_URL}/api/auth/referrals/getPatientReferredTransactions/${patient}`,
                {
                    skip: skip,
                    per_page: perPage,
                    sorton: column,
    ...
    const col = [
        ...
        // },
        {
            ...
            cell: (row) => row.ReferredToPharmacy ? "Pharmacy" : row.ReferredToPatient ? "Patient" : row.ReferredToDoctor ? "Doctor" : row.ReferredToLaboratory ? "Lab" : row.ReferredToHospital ? "Hospital" : "Patient",
            sortable: false,
        ...
        {
            name: "Referred To",
            cell: (row) => row.ReferredToPharmacy || row.ReferredToPatient || row.ReferredToDoctor || row.ReferredToLaboratory || row.ReferredToHospital || "NA",
            sortable: false,
            // sortField: "EmailPersonal",
            // minWidth: "150px",
            // maxWidth: "200px",
        ...
        },
        {
            name: "Date",
            selector: (row) => {
                const createdAt = row?.createdAt; // Assuming createdAt is a valid ISO8601 date string
                if (!createdAt) return ""; // Handle cases where createdAt is undefined or null

                // Splitting the ISO8601 date string to parse it manually
                const parts = createdAt.split(/[-T:.TZ]/);
    ...
    document.title = "Refer a Friend Point Transactions | Grace Labs";

    return (
        <React.Fragment>
            <div className="page-content">
                <Container fluid>
                    <BreadCrumb maintitle="Wallet" title="Refer a Friend Point Transactions" pageTitle="Wallet" />
                    <Row>
                        <Col lg={12}>
                            <Card>
                                <CardHeader>
                                    <Row className="g-4 mb-1">
                                        <Col className="col-sm" sm={6} lg={4} md={6}>
                                            <h2 className="card-title mb-0 fs-4 mt-2">Refer a Friend Point Transactions</h2>
                                        </Col>
...
export default ReferTransactions;
